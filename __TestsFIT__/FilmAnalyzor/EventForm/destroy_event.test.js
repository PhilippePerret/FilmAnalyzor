...
describe("Destruction d'un event à l'aide du formulaire d'édition", async function(){
  ...
  this.case("Le formulaire permet de détruire un event", async () => {
    ...
    sumary(`
      Fichier principal à corriger en cas d'erreur : app/js/common/FAnalyse/events.js
      `)
    ...
    // expect(d(`#banctime-tape #banctime-event-${event_id}`)).exists({onlyReturn:true})
    await waitFor(()=>{return $(`#banctime-tape #banctime-event-${event_id}`).length > 0})
    await remember(
    ...
    compare('nombre.ids', 'same')
    expect(current_analyse.ids[event_id], `ids[${event_id}]`).is_undefined()
    await compare('event.is.on.banctimeline', 'inverse')
    await compare('event.is.on.reader', 'inverse')
